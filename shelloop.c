#include "shell.h"

/**
 * mhsh - main loop
 * @info: parameter struct
 * @av: the argument vector
 * Return: 0 success
 */
int mhsh(info_t *info, char **av)
{
	ssize_t r = 0;
	int builtin_ret = 0;

	while (r != -1 && builtin_ret != -2)
	{
		clear_info(info);
		if (intactive(info))
			puts("$ ");
		eputchar(BUF_FLUSH);
		r = get_input(info);
		if (r != -1)
		{
			set_info(info, av);
			builtin_ret = finds_builtin(info);
			if (builtin_ret == -1)
				find_cmd(info);
		}
		else if (intactive(info))
			putchar('\n');
		free_info(info, 0);
	}
	write_hist(info);
	free_info(info, 1);
	if (!intactive(info) && info->status)
		exit(info->status);
	if (builtin_ret == -2)
	{
		if (info->err_num == -1)
			exit(info->status);
		exit(info->err_num);
	}
	return (builtin_ret);
}

/**
 * finds_builtin - finds command
 * @info: struct paparameter
 * Return: int
 */
int finds_builtin(info_t *info)
{
	int i, built_in_ret = -1;
	builtin_table builtintbl[] = {
		{"exit", myexit},
		{"env", cmyenv},
		{"help", _mychelp},
		{"history", _myhistory},
		{"setenv", mysetenvi},
		{"unsetenv", _dmyunsetenv},
		{"cd", mycd},
		{"alias", _myalias},
		{NULL, NULL}
	};

	for (i = 0; builtintbl[i].type; i++)
		if (_strcmp(info->argv[0], builtintbl[i].type) == 0)
		{
			info->line_count++;
			built_in_ret = builtintbl[i].func(info);
			break;
		}
	return (built_in_ret);
}

/**
 * find_cmd - finds a command
 * @info: struct parameter
 * Return: nothing
 */
void find_cmd(info_t *info)
{
	char *path = NULL;
	int i, k;

	info->path = info->argv[0];
	if (info->linecount_flag == 1)
	{
		info->line_count++;
		info->linecount_flag = 0;
	}
	for (i = 0, k = 0; info->arg[i]; i++)
		if (!s_delim(info->arg[i], " \t\n"))
			k++;
	if (!k)
		return;

	path = find_path(info, getenva(info, "PATH="), info->argv[0]);
	if (path)
	{
		info->path = path;
		forks_cmd(info);
	}
	else
	{
		if ((intactive(info) || getenva(info, "PATH=")
					|| info->argv[0][0] == '/') && is_cmd_f(info, info->argv[0]))
			forks_cmd(info);
		else if (*(info->arg) != '\n')
		{
			info->status = 127;
			printerror(info, "not found\n");
		}
	}
}

/**
 * forks_cmd - forks a an exec
 * @info: struct
 * Return: nothing
 */
void forks_cmd(info_t *info)
{
	pid_t child_pid;

	child_pid = fork();
	if (child_pid == -1)
	{
		perror("Error:");
		return;
	}
	if (child_pid == 0)
	{
		if (execve(info->path, info->argv, getstr_environ(info)) == -1)
		{
			free_info(info, 1);
			if (errno == EACCES)
				exit(126);
			exit(1);
		}
	}
	else
	{
		wait(&(info->status));
		if (WIFEXITED(info->status))
		{
			info->status = WEXITSTATUS(info->status);
			if (info->status == 126)
				printerror(info, "Permission denied\n");
		}
	}
}

